...
.. note::
   ...
   Django-tenants supports both Django's unittest-style test cases and pytest fixtures.
   For pytest users, see :doc:`pytest` for a more pytest-idiomatic approach to testing with tenants.
...
Django-tenants provides two approaches for testing:

1. **Django Test Cases** (this page): Traditional unittest-style test cases
2. **Pytest Fixtures** (:doc:`pytest`): Modern pytest-style fixtures

This page covers Django's test cases. For pytest users, see :doc:`pytest`.

Using TenantTestCase
^^^^^^^^^^^^^^^^^^^^
